...
class FakeTrack:
    def __init__(self, shot_id, segment_id, frame_range, embeddings=None, track_id=None):
        """
        frame_range: (first_frame, last_frame) inclusive
        embeddings: list[np.ndarray] (optionally empty/None)
        """
        self.shot_id = int(shot_id)
        self.segment_id = segment_id  # may be None for per-track grouping fallback
        self._first, self._last = map(int, frame_range)
        assert self._first <= self._last
        self.observations = [FakeObs(i) for i in range(self._first, self._last + 1)]
        self.embeddings = list(embeddings) if embeddings is not None else []
        self.track_id = track_id
        self.global_id = None  # assigned by resolver
        self.observations = [DummyObs(self._first)]
...
def test_time_first_is_deterministic_under_shuffle(tmp_path):
    ...
    res1 = GlobalIdentityResolver(embedding_threshold=0.65,
                                  debug_dump_path=str(tmp_path/"a.json"))
    ...
    res2 = GlobalIdentityResolver(embedding_threshold=0.65,
                                  debug_dump_path=str(tmp_path/"b.json"))
...
def test_seed_map_preserves_ids():
    tracks = build_tracks()
    # Simulate a pre-resume known ID: (shot1,seg0) must be gid=7
    seed_map = {(1,0): 7}
    res = GlobalIdentityResolver(embedding_threshold=0.65, seed_map=seed_map)
    res.resolve_global_ids(tracks)
    assert tracks[0].global_id == 7
    # Others should be >= 0 and distinct from 7
    gids = {t.global_id for t in tracks}
    assert 7 in gids and len(gids) == len(tracks)
