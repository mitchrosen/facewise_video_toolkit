...
def test_load_and_anchor_collectors_prefers_embedding_safe_anchor(tmp_path: Path):
    ...
    # Frame trim should use the safe frame *inclusively*.
    # (The anchor frame is the latest embedded frame, so it is part of the durable boundary.)
    assert obs.trim_frame_calls and obs.trim_frame_calls[-1] == 170
...
class _StubAggregator:
    def snapshot_open_tracks(self):
        return []


def test_checkpoint_now_is_noop_when_write_disabled(tmp_path: Path):
    dummy_video = tmp_path / "dummy.mp4"
    dummy_video.touch()
